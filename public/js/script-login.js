...
$( "form > p" ).addClass( "inputGroup" );
$( '<p class="helper helper1">email@domain.com</p>' ).appendTo( "form > p:first label" );
$( '<label id="showPasswordToggle" for="showPasswordCheck">Show <input id="showPasswordCheck" type="checkbox"/><div class="indicator"></div> </label>' ).appendTo( "form > p:nth-child(3) label" );

 $("form > p").each(function(i) {
      $(this).addClass("inputGroup"+(i+1));
    });

    
    $( ".login #login_error, .login .message" ).dialog({
        autoOpen: true,
        draggable: false,
        modal: true,
        width: 'auto',
...
var emailLabel = document.querySelector('.inputGroup1 label'), 
email = document.querySelector('#user_login'), 
passwordLabel = document.querySelector('.inputGroup2 label'), 
...
function initLoginForm() {
	...
	email.addEventListener('input', onEmailFocus);
